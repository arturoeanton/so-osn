...
| **🐚 FASE 13.1 — BusyBox ash + login mode + .bashrc-style /home/.ashrc** (default shell de sistema): `/bin/busybox` 1.36.1 linkeado contra musl reemplaza a `shellsrv` como init shell. `proc_execve("/bin/busybox", "sh -l", envp)` con envp pre-poblado `PATH=/bin HOME=/home HISTFILE=/home/.ash_history HISTSIZE=500 TERM=linux` (HISTFILE en envp, NO en /etc/profile, porque ash lo lee antes de cmdloop). Split estilo bash: `/etc/profile` (sourced ONCE en login) sólo trae exports + `ENV=/home/.ashrc`; `/home/.ashrc` (sourced cada shell interactiva via $ENV, mirror exacto de `~/.bashrc`) trae banner ASCII "OSnOS", PS1 verde `osnos:\w# ` y aliases (`ll la l .. h cls`). Usuario edita `.ashrc` con `ovi /home/.ashrc` sin recompilar. **History persiste cross-reboot a `/home/.ash_history`** (FAT16): rebuild de busybox con `FEATURE_EDITING_SAVEHISTORY=y` + `SAVE_ON_EXIT=y` + `HISTORY=500` + `FANCY_PROMPT=y`. Up/↓ arrow recupera comandos de sesiones anteriores. Bug del wrapper `osnos-cc-wrapper.sh` corregido: necesitaba `-target x86_64-unknown-none-elf` (sin él clang on macOS producía Mach-O ARM64) + `-U__APPLE__ -D__linux__` (sin él busybox `platform.h` tomaba branch BSD con `<machine/endian.h>` macOS-only) + musl includes injectados + filtrado link flags. Mismo rebuild trajo **~30 applets nuevos: `vi awk sed find diff patch hexdump more dd df du stat base64 md5sum sha1sum sha256sum cksum bc dc xargs tac factor fold expand rev strings readlink realpath timeout`** — accesibles transparente via aliases en `/home/.ashrc` (FAT16 no soporta symlinks `/bin/vi → /bin/busybox` estilo Linux, así que cada applet se mapea con `alias vi='busybox vi'`). El `vi` modal completo (i/a/o, hjkl, dd, :w :q, /search) reemplaza al `ovi` artesanal de FASE TUI; esa fase queda **superseded por BusyBox** — la suite POSIX completa ya está, no hay que re-implementarla. **4 bugs kernel fixed** para que ash sobreviva: **(1) restart_syscall pattern**: `sys_read`/`sys_poll` rebobinan iret RIP 2 bytes + saved_rax = syscall_nr para que el CPU re-ejecute el syscall al despertar, en vez de longjump-with-rax=0 que ash interpretaba como EOF → exit(0) → infinite respawn loop. Patrón POSIX correcto. **(2) Syscall numbers** osnos-specific 260-268 → 510-518 (chocaban con Linux #262=newfstatat que musl `stat()` invoca). Nuevos: `SYS_LSTAT=6`, `SYS_OPENAT=257`, `SYS_NEWFSTATAT=262`, `SYS_EXIT_GROUP=231`. **(3) `sys_stat` copy_from_user** byte-a-byte hasta NUL (antes pedía 128 B y faulteaba con paths cortos al borde de página). **(4) `VFS_MAX_MOUNTS`** 8→16 (con `/home` aliasfs entraban 9 mounts, el extra perdía). Banner ASCII + welcome aparecen en cada boot/respawn. ↑/↓ recall in-memory history out-of-the-box (FEATURE_EDITING ya estaba on); persistencia cross-reboot a `/home/.ash_history` es best-effort según binario actual de busybox. `shellsrv` queda como fallback si `/bin/busybox` falta (diskless boot). | ✅ |
