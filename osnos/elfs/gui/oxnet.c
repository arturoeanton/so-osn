/*
 * oxnet — live networking inspector.
 *
 *   - top:    /sys/net   (interfaces / sockets)
 *   - bottom: /sys/arp   (ARP cache)
 *
 * Same shape as oxmem/oxipc. Refreshes every 1 s.
 */
...
static panel_t g_panels[2] = {
    { .path = "/sys/net", .title = "Net (ifaces + sockets)" },
    { .path = "/sys/arp", .title = "ARP cache"              },
};

#define COL_FG       OX_RGB( 20,  20,  20)
#define COL_HDR_BG   OX_RGB(232, 232, 232)
#define COL_BORDER   OX_RGB( 80,  80,  80)
#define COL_PANEL_HDR    OX_RGB( 60, 140,  90)
...
static void render(void) {
    ...
    ox_draw_text(g_win, 10, 10, "Network  —  refresh every 1s",
                 OX_RGB(30, 30, 30));
