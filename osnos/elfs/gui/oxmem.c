/*
 * oxmem — live memory + task viewer.
 *
 * Splits the window into two scrollable panels:
 *   - top:    /sys/meminfo
 *   - bottom: /sys/tasks
 * Auto-refreshes every 1 second so you can watch kernel state move.
 */
...
#define COL_PANEL_HDR OX_RGB(102, 152, 203)
...
static void draw_panel(panel_t *p) {
    /* Panel header (blue bar above content). */
...
static void render(void) {
    ...
    ox_draw_text(g_win, 10, 10, "Memory & Tasks  —  refresh every 1s",
                 OX_RGB(30, 30, 30));
...
int main(int argc, char **argv) {
    ...
    g_win = ox_window_create_resizable(g_w, g_h, "Memory & Tasks");
    if (g_win < 0) return 1;
    layout();
    slurp(&g_panels[0]);
    slurp(&g_panels[1]);
