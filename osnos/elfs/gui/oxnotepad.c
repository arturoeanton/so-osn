...
int main(int argc, char **argv) {
    ...
    while (!quit) {
        ...
        /* Ctrl combos — save / quit / clipboard / select all. */
        if (ev.mods & OX_MOD_CTRL) {
            int a = ev.ascii;
            /* Some keyboard layers strip the high bit and just send the
             * letter; others send the ^X control byte (ascii 1..26). */
            if (a == 's' || a == 'S' || a == 0x13) {
                save_file(); render(); continue;
            }
            if (a == 'q' || a == 'Q' || a == 0x11) {
