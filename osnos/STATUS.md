...
|------|-----------|------------|
| **Polish + Fase 2.1 + cleanup** | (1) Shellsrv `cd` con resolución de paths relativos `..` `.`. (2) `ls /` muestra mount points (/bin /dev /sys /home /sd) sintéticos via vfs_readdir. (3) `/bin/clear` + `/bin/tree` ELFs. (4) ovi output buffering (16KB single-write per render) — evita saturar IPC console queue de 64 slots. (5) **FAT16 dir-chain extension** — extend_dir_chain alloca nuevo cluster y lo encadena cuando find_free_dir_slots_run hit ENOSPC; resuelve el cap de ~9 entries por subdir. (6) Disk seed completo re-habilitado (~60 ELFs en /sd/bin). (7) Coreutils completos: env, wc, pwd, uname, basename, dirname, tail, seq, yes, tee, date, printf, grep, sort, uniq, cut, tr, banner, which, clear, tree | 350 |
