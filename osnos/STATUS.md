...
en vivo — recon listo para la línea gráfica (cursor overlay,
window system, eventual TinyX). Todo verificado por **18/18
tests automatizados** vía `/bin/alltest`. Bonus: `/bin/poweroff`
+ `/bin/reboot` (ACPI S5 / 8042 reset), `tail -f` poll-based,
`/bin/readelf -a/-l/-S/-h` para inspeccionar binarios.
