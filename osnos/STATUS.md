...
|------|-----------|------------|
| **FASE 13.3 — SQLite 3.45.2 port (cuarto lenguaje self-host: SQL)** | (1) **`vendor/sqlite/`** — amalgamation 3.45.2 (sqlite3.c ~250K LOC + shell.c ~30K LOC + sqlite3.h, single-file build pattern). Descargado de www.sqlite.org. Linkeado contra musl 1.2.5. Output `/bin/sqlite3` ~5 MB ELF estático. (2) **4 syscalls nuevos en kernel** para que SQLite bootee: `SYS_FSYNC=74` + `SYS_FDATASYNC=75` (stubs que retornan 0 — FAT16 escribe sync por default, no hay write-back cache que flushear), `SYS_FTRUNCATE=77` (truncar regular file a tamaño N, sqlite lo usa para journal files: lee primeros N bytes, pad con zeros si file < N, re-escribe via vfs_write), `SYS_GETTIMEOFDAY=96` (alias de clock_gettime con conversion a `{sec, usec}`), `SYS_GETRANDOM=318` (PRNG xorshift seeded por timer — no cryptographic-grade pero sqlite solo lo usa para entropy de temp file naming). (3) **`sys_fcntl` extendido**: `F_SETLK=6`, `F_GETLK=5`, `F_SETLKW=7`, `F_OFD_*=36-38` retornan 0 (pretend lock acquired) — osnos es effectivamente single-process así que advisory locks no aplican; sin esto, sqlite open() falla con disk I/O error porque no puede acquirir el lock POSIX. También `F_DUPFD_CLOEXEC=1030` mappeado a F_DUPFD + set CLOEXEC. (4) **Bumps de límites del kernel**: `EXEC_VFS_BLOB_MAX` 2 MiB → 16 MiB en `src/proc/exec.c` (sqlite ELF es 5 MB, antes execve abortaba con E2BIG); `KHEAP_MAX_BYTES` 4 MiB → 32 MiB en `src/micro/kmalloc.c` (kmalloc del blob ELF rebotaba "Out of memory"). (5) **SQLite CFLAGS de port**: `-DSQLITE_THREADSAFE=0` (single-task per shell), `-DSQLITE_OMIT_LOAD_EXTENSION` (no dlopen), `-DSQLITE_OMIT_WAL` (write-ahead-log requiere shared memory que no tenemos), `-DSQLITE_DEFAULT_LOCKING_MODE=1` (exclusive — minimiza fcntl trips), `-DSQLITE_DEFAULT_TEMP_STORE=2` (temp tables en memoria), `-DSQLITE_NO_SYNC=1` (skip fsync, ya stubbed), `-DSQLITE_DEFAULT_MMAP_SIZE=0` + `-DSQLITE_MAX_MMAP_SIZE=0` (sin mmap I/O — fallback a read/write), `-DSQLITE_OMIT_SHARED_CACHE`, `-DSQLITE_DEFAULT_FOREIGN_KEYS=1`, `-DHAVE_USLEEP=1`. Mantenemos SET_AUTHORIZER y PROGRESS_HANDLER porque shell.c los necesita. (6) **`res/demo.sql` + `res/demo.db` shipped**: GNUmakefile auto-regenera `demo.db` (28 KB) al build via `sqlite3 host < res/demo.sql` y la copia a `/home/demo.db` + `/home/demo.sql` en sd.img. Schema: tabla `books` (15 entries de libros clásicos de CS — K&R, SICP, Tanenbaum, Knuth-style refs), `users` (4), `checkouts` (6, con FK refs), vista `v_open_checkouts`, indices `idx_books_year` + `idx_books_author`. (7) **`/home/.ashrc` extendido** con `alias sqlite3='busybox sqlite3'`... wait, sqlite3 es nuestro ELF estandalone (no busybox applet) así que está en /bin/sqlite3 directo. (8) **Verificación end-to-end**: `sqlite3 --version` → "3.45.2 2024-03-12 ...". `sqlite3 :memory: < q.sql` con `SELECT 99` → "99". `sqlite3 /home/demo.db < q.sql` con `SELECT title, year FROM books ORDER BY year LIMIT 5` → lista correcta: "The Mythical Man-Month\|1975", "The C Programming Language\|1978", "SICP\|1985", "The Design of the UNIX OS\|1986", "Compilers...\|1986". **Cuarto lenguaje self-host** en osnos: C (TCC) + Lua + jq + **SQL (SQLite)**. (11) **Bug REPL backspace visual fixed** (`src/micro/tty.c`): mismo problema raíz que echo — `tty_echo_erase` llamaba `framebuffer_backspace` directo (solo movía cursor, no clearaba el char visualmente; tampoco mirroreaba a serial). Fix: ahora envía la secuencia clásica `"\b \b"` via `framebuffer_write_bytes` (cursor atrás, sobreescribe con espacio, cursor atrás otra vez). Bonus: la secuencia llega también a serial así nc/headless mirror lo refleja. Verificado: `echo abc[DEL][DEL][DEL]world` → resultado `world` (line buffer correcto), display visual elimina los chars borrados. (10) **Bug REPL echo fixed — `tty_echo_char` ahora usa `framebuffer_write_bytes`** (`src/micro/tty.c`): el line discipline tenía dos paths divergentes: apps escribían a stdout via `consrv → /dev/fb → framebuffer_write_bytes` (con cursor + serial mirror), mientras que el echo de keystroke del kernel usaba `framebuffer_draw_string` DIRECTO (sin serial mirror, distinto path de cursor). Resultado: en REPLs de sqlite/lua donde el terminal está en canonical+echo, el usuario tipeaba y los chars no aparecían — el output de la app via consrv y el echo del kernel se desincronizaban en cursor. Fix: tty_echo_char ahora también usa `framebuffer_write_bytes` (mismo path que apps), echo aparece junto al prompt y se mirrorea a serial. Bonus: ahora el anti-clobber `tty_set_termios` (que rechaza disable de ECHO mientras hay children vivos) cumple su rol como segunda capa. Verificado: `sqlite3 :memory:` con `SELECT 99;` muestra el SELECT mientras se tipea; lua REPL `print(2+2)` igual. **REPL interactivo de sqlite, lua, y cualquier programa POSIX que lea de TTY ahora muestra echo al usuario en tiempo real.** (9) **Bug crítico fixed — FS_BASE save/restore + reset en execve** (`src/micro/task.{c,h}` + `src/proc/exec.c` + `src/micro/syscall.c`): el primer test de sqlite3 reveló que **ash (busybox, pid=6) page-faulteaba en `__errno_location`** después de que sqlite3 (child) terminaba. Causa raíz: `arch_prctl(ARCH_SET_FS, addr)` (que musl usa para TLS) escribe `wrmsr MSR_FS_BASE` globalmente en el CPU — NO es per-task automáticamente. Tres fixes interconectados: **(a)** agregado `uint64_t fs_base` a `task_t`; `task_run_next` hace `rdmsr` al guardar contexto saliente y `wrmsr` al cargar entrante. **(b)** `proc_execve` (in-place exec + new task variant) resetea `fs_base=0` porque el address space cambia → musl `__init_libc` reseteará vía arch_prctl al arrancar. Sin (b), ash → fork → execve → child hereda el fs_base del parent que apuntaba a TLS válida en pml4 del parent pero INVÁLIDA en el pml4 nuevo del child → cada busybox applet (ls, find, sqlite3, etc.) page-fault inmediato en el primer access a errno via %fs:offset. **(c)** `sys_fork` copia explícitamente `fs_base` del parent al child (mismo address space clonado, TLS sigue siendo válida hasta que child haga execve). Verificado end-to-end: **fault count = 0** después de muchas invocaciones de sqlite3/find/md5sum/ls. ash sobrevive con su history, env, .ashrc intactos. **Bug known restante**: sqlite3 con SQL en argv tiene argv passing issues (workaround: stdin redirect). | 600 |
| **FASE 13.1 — BusyBox ash + login mode + .bashrc-style /home/.ashrc** | (1) **`vendor/busybox/`** — BusyBox 1.36.1 vendored, linkeado contra musl 1.2.5 via `osnos-cc-wrapper.sh` (clang frontend → `ld.lld -m elf_x86_64 -static -no-pie --gc-sections crt1.o ... libc.a`). Resulting `busybox_unstripped` ~1.4 MB con ~30 applets enabled: `sh ash echo cat ls ls find sed grep cut tr sort uniq head tail wc cp mv rm mkdir touch chmod md5sum expr cksum cpio basename dirname date dd du tee env pwd which printf seq yes test true false sleep less env uname`. (2) **BUG CRÍTICO #1 — restart_syscall pattern**: `sys_read` y `sys_poll` originalmente loopeaban con `sys_nanosleep()` para esperar input; pero `sys_nanosleep` hace `sched_resume_jump()` (longjmp al scheduler, NEVER returns) y deja al task con `saved_rax=0` apuntando al RIP user-space DESPUÉS del syscall. Resultado: ash llamaba `read(0)`, kernel longjumpeaba, ash recibía `read=0` que interpreta como EOF → exit(0) → watchdog respawn → infinite loop (52+ banners/30s). Fix: nuevo helper `block_restart_syscall(wakeup_ms, syscall_nr)` que stamp el iret frame con `rip -= 2` (rebobina 2 bytes — coincidente para `syscall 0F 05` y `int 0x80 CD 80`) + `saved_rax = syscall_nr` (SYS_READ=0, SYS_POLL=7). Al despertar la CPU re-ejecuta el syscall original con args originales en rdi/rsi/rdx → chequea readiness de nuevo. Esto es el patrón POSIX `restart_syscall` en kernel — sin él, libcs que no auto-retrytan en EAGAIN (musl, fopen-via-write libs) fallan inmediato. (3) **BUG CRÍTICO #2 — colisión de syscall numbers**: osnos-specific syscalls vivían en 260-268 (`SYS_IPC_SEND/RECV/SERVICE_REGISTER/LOOKUP/TTY_INPUT/TASKINFO/SPAWN/SET_FG/RESUME`) — chocaban con Linux x86_64 syscalls 260=fchownat, **262=newfstatat (que musl `stat()` invoca)**, 263=unlinkat, etc. musl `stat("/")` retornaba lo que respondía SERVICE_REGISTER → mode bits basura → ls/find/cp todos rotos. Movidos a 510-518 (kernel `src/micro/syscall.h` + libc `lib/libc/syscall.h` + `lib/libc/include/osnos_ipc.h`). Nuevos mappings agregados: `SYS_LSTAT=6` (alias de sys_stat, osnos no tiene symlinks), `SYS_OPENAT=257` (sys_open con dirfd ignorado), `SYS_NEWFSTATAT=262` (sys_stat con dirfd ignorado), `SYS_EXIT_GROUP=231` (alias de sys_exit). (4) **BUG CRÍTICO #3 — `sys_stat` faulteaba con paths cortos**: `copy_from_user(kpath, path, OSNOS_PATH_MAX)` pedía 128 bytes a partir de `path`, faulteando para strings al final de página como `"/"`. ls llamaba stat("/") → EFAULT → "cannot open /". Fix: copia byte-a-byte hasta NUL terminator. (5) **BUG CRÍTICO #4 — `VFS_MAX_MOUNTS=8` insuficiente**: con `/, /sys, /dev, /sd, /bin, /lib, /usr, /etc, /home` sumamos 9 mounts — `/home` no entraba y `find_mount("/home")` devolvía NULL. Bumpado a 16. (6) **Login shell + split /etc/profile + /home/.ashrc** (estilo `.bashrc`): `proc_execve("/bin/busybox", "sh -l", shell_envp)` con `shell_envp = {PATH=/bin, HOME=/home, HISTFILE=/home/.ash_history, HISTSIZE=500, TERM=linux}` (HISTFILE en envp porque ash lo lee ANTES de cmdloop, donde se sourcearía /etc/profile — fix de un bug que dejaba la HISTFILE no-detectada). Watchdog `server_respawn_tick` usa el mismo envp. Bootstrap seedea **dos archivos separados, mirror exacto del convention bash**: (a) `/etc/profile` (sourced ONCE on login) — solo `export PATH/HOME/HISTFILE/HISTSIZE/TERM` + `export ENV=/home/.ashrc`. NO banner, NO PS1 — esos son interactive concerns. (b) `/home/.ashrc` (sourced en CADA shell interactiva via $ENV, igual que ~/.bashrc) — `export PS1='osnos:\w# '`, aliases (`ll`, `la`, `l`, `..`, `h`, `cls`), `/bin/banner osnos`, mensaje de bienvenida. **Bug fixed**: el seed de `/home/.ashrc` originalmente estaba en el bloque /etc del bootstrap antes de que el aliasfs `/home → /sd/home` estuviera montado → escribía a ramfs sin efecto. Movido al bloque /home post-mount. **Bug fixed**: el `/etc/profile` y `$ENV` apuntaban al MISMO archivo (banner duplicado en cada boot). Split lo elimina — banner aparece EXACTAMENTE 1 vez por shell. Resultado: cada boot muestra el ASCII art "OSnOS" + welcome + prompt verde `osnos:/#`. Usuario puede editar `/home/.ashrc` con `ovi /home/.ashrc` sin recompilar nada. (7) **History line-editing + persistence**: rebuild de busybox con `FEATURE_EDITING=y` + `FEATURE_EDITING_HISTORY=500` + `FEATURE_EDITING_SAVEHISTORY=y` + `FEATURE_EDITING_SAVE_ON_EXIT=y` + `FEATURE_EDITING_FANCY_PROMPT=y`. Up/down arrow recall in-memory + PS1 `\w` expansion funciona; **`/home/.ash_history` se escribe en `exit` y persiste en FAT16 cross-reboot** (al respawn / próximo boot, ash carga el archivo y up-arrow recupera comandos previos). Bug del rebuild que costó: `osnos-cc-wrapper.sh` necesitaba **`-target x86_64-unknown-none-elf`** en compile-mode (sin él, clang on macOS produce Mach-O ARM64 nativo, no ELF x86_64) + **`-U__APPLE__ -D__linux__`** (sin él, `include/platform.h` toma rama BSD que requiere `<machine/endian.h>` macOS-only) + injected musl includes via `-isystem` (busybox no las pasa por default) + filtrado de flags clang-only del link path (`-finline-limit`, `-falign-jumps/labels/loops`, `-Wp,*`, etc) + branch separada de preprocess-mode (`-E -xc -MM -dM`) para que feature-detect probes no caigan en el link path. (8) **Más applets BusyBox habilitados** (post-rebuild fix): `vi awk sed find diff patch hexdump more dd df du stat readlink realpath base64 md5sum sha1sum sha256sum cksum timeout xargs rev strings tac factor fold expand bc dc`. Total ~60 applets en el binario (era ~30). **STANDALONE_SHELL deshabilitado** porque tenía un bug de dispatch multi-arg en osnos (`sed s/x/y/ file` → ": applet not found" porque argv shift se rompe). En vez de eso, `/home/.ashrc` define aliases `alias vi='busybox vi'`, `alias sed='busybox sed'`, etc. para ~30 applets — efecto transparente: `vi foo.txt` invoca `busybox vi foo.txt`. FAT16 no soporta symlinks así que el approach "instalar /bin/vi → /bin/busybox" estilo Linux no aplica; los aliases son el equivalente clean. **vi (vim modal) ahora disponible**: reemplaza a `ovi` artesanal con todo lo que se espera (i/a/o, hjkl, dd, :w :q, /search, yank, undo). Misma config trajo `bc` (calculadora arbitrary-precision) y `base64`, `hexdump`, `stat -c`, `find -exec`, etc. (8) **Smoke verificado end-to-end**: `echo HELLO`, `echo arith=$((100*7))` → 700, `for i in a b c; do echo $i; done` → a/b/c, `ls /` → sys/dev/sd/bin/lib/usr/etc/home/, `ls /etc` → passwd group hosts (vía aliasfs), `cat /home/README.TXT`, `echo hi > /home/test.txt` (persiste en FAT16), `ls /bin \| head -n 5`, `tr a-z A-Z`, `sort`, `wc -l`, `uname -srm`. Banner aparece al boot. Default shell de osnos pasa de `shellsrv` (interno) a `busybox sh` (POSIX-compliant). `shellsrv` sigue disponible como fallback si `/bin/busybox` falta (diskless boot). | 800 |
...
| Self-hosting | `tcc` (TinyCC 0.9.27, ~1 MB) compila C nativo, ELFs estáticos runnable (FASE 11.0). `lua` (Lua 5.4.7, ~1.2 MB) interprete + REPL + scripts (FASE 11.2). `jq` (jq 1.7.1, ~1.1 MB) filter/transformer JSON con paths + builtins + arithmetic + pipes (FASE 11.3). `sqlite3` (SQLite 3.45.2, ~5 MB, musl-linked) SQL engine completo + demo.db (15 books, 4 users) shipped en /home (FASE 13.3). | 4 | osnos self-compila C, ejecuta Lua, filtra JSON con jq, y corre SQL con SQLite |
...
### FASE 12 (roadmap original) — TUI potente — SUPERSEDED por BusyBox
Esta fase queda **fuera de scope** desde FASE 13.1: BusyBox 1.36.1
trae todo lo útil de TUI sin que tengamos que escribirlo. En vez de
re-implementar mini-mc, less custom, ovi con highlights, etc.,
exponemos los applets de BB con aliases en `/home/.ashrc`:
- ✓ **`vi`** — full vi modal (h j k l, i/a/o, dd, :w :q, /search,
  yank, undo). Reemplaza al `ovi` artesanal.
- ✓ **`less`** — pager con search + scrollback (el `/bin/less`
  custom de antes sigue ahí; el de BB también funciona via alias).
- ✓ **`sed awk find diff patch grep cut sort uniq head tail wc tr
  base64 md5sum sha1sum sha256sum cksum stat dd dc bc hexdump more
  tac fold expand factor xargs rev readlink realpath strings
  timeout`** — toda la suite POSIX accessible. Wrappers en `.ashrc`:
  `alias vi='busybox vi'`, etc. Sin esto, los applets están en el
  binario pero ash no los encuentra en `$PATH` (FAT16 no tiene
  symlinks para linkear `/bin/vi → /bin/busybox`).
- ☐ "Norton Commander" dos paneles: el file browser GUI `/bin/oxfiles`
  (FASE 12.1) cubre el caso click; un mini-mc TUI texto-only sigue
  pendiente pero baja prioridad ya que vi + find + cp + mv en ash
  cubre el flujo CLI.
